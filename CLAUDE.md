...
- **Chapters run ~250–450 lines**, and up to ~600 when a chapter has to build an entire vocabulary from zero (`reading-criterion.md` is 585). Length is the *last* thing to optimise: never drop a definition or a worked example to hit a line count. A chapter that assumes vocabulary is not shorter, it is unfinished.
